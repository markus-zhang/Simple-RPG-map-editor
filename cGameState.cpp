...
void cGameState::HandleInput(cStateManager* state_manager)
{
	...
	if ( m_Input->KeyHeld(SDL_SCANCODE_RIGHT))
	{
		if (m_XBias < EDITOR_BIAS_X_MAX)
		{
			m_XBias += 2;	// Scroll to right
			cout << "You changed xbias to " << m_XBias << "\n";
		};
	}
	if ( m_Input->KeyHeld(SDL_SCANCODE_LEFT))
	{
		if (m_XBias < EDITOR_BIAS_X_MAX)
		{
			m_XBias -= 2;	// Scroll to left
			cout << "You changed xbias to " << m_XBias << "\n";
		};
	}
	/*if ( m_Input->KeyHeld(SDL_SCANCODE_LEFT) && PlayerCanMoveRight() )
	{
		m_Player.MoveRight();
	}
...
//string cGameState::GetEntityIDByLocation()
//{
...
void cGameState::Load(const char* mapfile, const char* entityfile)
{
	m_MapVector.clear();
	//Load Map
